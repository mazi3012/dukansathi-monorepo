...
async def generate_telegram_token(request: Request):
    """
    Generate a short-lived 6-char OTP code for linking Telegram to a web account.
    Called by the frontend after user is authenticated.
    Returns: { token: "A3K8X2", expires_in_seconds: 600 }
    """
    ...
    # Generate a 6-char alphanumeric code (easy to type)
    ...
    try:
        ...
        # Insert new token
        supabase.table("telegram_connect_tokens").insert({
            "user_id": user_id,
            "token": code,
        ...
        logger.error(f"Error saving telegram token: {e}")
        raise HTTPException(status_code=500, detail="Could not generate token")
    
    return {"token": code, "expires_in_seconds": 600}

@app.post("/api/telegram/verify-token")
async def verify_telegram_token(request: Request):
    """
    Called by the Telegram bot to exchange a 6-char code for a user_id.
    Saves the telegram_chat_id → user_id mapping in telegram_users.
    Body: { token: "A3K8X2", telegram_chat_id: 12345678, telegram_username: "handle" }
    Returns: { success: true, user_id: "uuid" }
    """
    body = await request.json()
    code = body.get("token", "").upper().strip()
    ...
    try:
        ...
        if not result.data:
            return {"success": False, "error": "Invalid code. Please generate a new one from the app."}
        
        row = result.data[0]
        
        if row["used"]:
            return {"success": False, "error": "This code has already been used. Please generate a new one."}
        
        if row["expires_at"] < now:
            return {"success": False, "error": "This code has expired. Please generate a new one from the app."}
        
        user_id = row["user_id"]
        
        # Save the Telegram → user mapping
        ...
        logger.info(f"Telegram account linked: chat_id={chat_id} → user_id={user_id}")
